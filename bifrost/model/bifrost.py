...
def get_bifrost_config(size: str = "base") -> Dict[str, Any]:
    """
    Get predefined BIFROST configuration.

    Args:
        size: Model size ('small', 'base', or 'large')

    Returns:
        Model configuration dictionary
    """
    if size not in BIFROST_CONFIGS:
        raise ValueError(f"Unknown model size: {size}")

    config = BIFROST_CONFIGS[size].copy()
    # Add common defaults
    config.update(
        {
            "num_token_types": 7,  # property, element, count, spacegroup, wyckoff, coordinate, lattice
        }
    )
